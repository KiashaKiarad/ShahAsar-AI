...
test("clean file moves atomically out of quarantine only after scanner, parser and extraction approval", async () => {
  ...
  try {
    ...
    const pipeline = createUploadPipeline({
      ...
      documentExtractor: async () => ({ type: "pdf", text: "متن قرارداد", sha256: require("crypto").createHash("sha256").update(pdf).digest("hex") })
    });
    ...
    assert.equal(result.extraction.text, "متن قرارداد");
...
test("malware or unavailable scan never reaches clean storage", async () => {
  ...
  try {
    const pipeline = createUploadPipeline({
      quarantineDir,
      cleanDir,
      virusScanner: async () => ({ clean: false })
    });
    await assert.rejects(
      pipeline.accept({ userId: "user-1", filename: "contract.txt", mimeType: "text/plain", size: 9, buffer: Buffer.from("legal text") }),
...
test("parser rejection also removes the quarantined file", async () => {
  ...
  try {
    const pipeline = createUploadPipeline({
      quarantineDir,
      cleanDir,
      virusScanner: async () => ({ clean: true }),
      documentValidator: () => { throw new Error("PARSER_REJECTED_DOCUMENT"); }
    });
    await assert.rejects(
      pipeline.accept({ userId: "user-1", filename: "contract.txt", mimeType: "text/plain", size: 9, buffer: Buffer.from("legal text") }),
...
test("extraction rejection also removes the quarantined file", async () => {
  ...
  try {
    const pipeline = createUploadPipeline({
      ...
      cleanDir,
      virusScanner: async () => ({ clean: true }),
      documentExtractor: async () => { throw new Error("EXTRACTION_REJECTED_DOCUMENT"); }
    });
    await assert.rejects(
      pipeline.accept({ userId: "user-1", filename: "contract.txt", mimeType: "text/plain", size: 9, buffer: Buffer.from("legal text") }),
      /EXTRACTION_REJECTED_DOCUMENT/
    );
    assert.equal((await fs.promises.readdir(cleanDir).catch(() => [])).length, 0);
    assert.equal((await fs.promises.readdir(quarantineDir).catch(() => [])).length, 0);
  } finally {
